    ...
    return (
      <div className="auth-container">
        <div className="auth-box">
          <h2 style={{textAlign:'center', fontFamily:'cursive', marginBottom: 24, color:'#262626'}}>Pixels</h2>
          {!resetMode ? (
            <>
              <form onSubmit={handleAuth}>
                ...
                <button type="button" style={{marginTop:8, width:'100%', background:'#6c47ff', color:'#fff', border:'none', borderRadius:8, padding:'10px 0', fontWeight:600, fontSize:16, cursor:'pointer'}} onClick={() => {
                  setEmail('demo@gmail.com');
                  setPassword('demo@gmail.com');
                  setIsSignUp(false);
                  setTimeout(() => {
                    document.querySelector('.auth-box form button[type="submit"]').click();
                  }, 100);
                }}>Demo User</button>
...
function HomePage({ user, posts, loading }) {
  ...
  function CreatePostModal({ user, onClose, onCreate }) {
    ...
    // Restrict demo user from posting
    ...
    return (
      <div style={{position:'fixed',top:0,left:0,width:'100vw',height:'100vh',background:'rgba(44,24,74,0.18)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}}>
        <div style={{background:'#fff',borderRadius:18,padding:32,minWidth:320,maxWidth:360,boxShadow:'0 8px 32px #a084e822',position:'relative'}}>
          ...
          {isDemoUser ? (
            <div style={{color:'#e74c3c',textAlign:'center',fontWeight:600,fontSize:17,padding:'32px 0'}}>Posting is disabled for the demo user.</div>
          ) : (
          <>
          {step === 0 && (
            <div style={{display:'flex',flexDirection:'column',gap:18}}>
